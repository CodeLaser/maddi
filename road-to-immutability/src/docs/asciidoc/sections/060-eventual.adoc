...
analyzer.
The current engine reads `@Mark`, `@Only`, `@TestMark` and the `after="..."` parameters as _contracts_, and
stores them as properties, but does not yet compute them; `@BeforeMark` is not read yet.
