...
life cycle they describe — are part of the design, and were implemented in an earlier generation of the
analyzer.
The current engine does not compute them; the chapter is retained because the design remains valid and
the pattern is central to how the analyzer's own support classes are written
(see <<in-the-analyzer>>).
====
