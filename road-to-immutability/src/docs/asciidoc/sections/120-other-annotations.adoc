...
Everything after them is **not implemented** at present:
...
* {finalizer} is likewise read as a contract, and none of its rules are enforced;
...
NOTE: *Not implemented.* {finalizer} is read as a contract and shown in output, but none of the three rules
below are enforced by the analyzer.
