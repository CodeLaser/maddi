...
More on this later, but here is a first example of a type with eventually final fields:
...
Moreover, it is easy to see that final fields alone are not enough to guarantee what we intuitively may think immutability
stands for:
