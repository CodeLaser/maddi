...
If we were to assume that all types in the same package are fully visible to the analyzer at the time of analysis, we could consider extending the rules to analyze all types in the package at the same time, as we did for nested types inside a primary type.
...
Even if inheritance were the only criterion used to define this access level, we would not allow it, because the child class can be invisible to the analyzer at the time of analysis of the parent.
