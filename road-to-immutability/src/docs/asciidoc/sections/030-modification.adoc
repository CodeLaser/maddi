...
non-private non-constructor method, applies at least one assignment or modifying method to this field's object graph.
****
...
* `E` because it is of an unbounded generic type, it has the same methods available as `Object`.
