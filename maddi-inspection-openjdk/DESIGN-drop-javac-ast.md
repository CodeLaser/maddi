...
the extract-interface engine on very large single-SCC projects (closed-core: 23,394 types, 661,633
...
- server/main (4,875 types): `com.sun.tools.javac.*` = **407 MB** (`JCTree$JCIdent` 41M,
  ...
  ~5 GB live set. On a project several times larger (closed-core), the absolute figure scales up.
...
source but absent from both the hints archive and the loaded set).
  ...
  - **Hard prerequisite: the hints load must SUCCEED.** If it is skipped or partial (e.g. closed-core's
...
At server/main scale this frees ~407 MB; at closed-core scale (and the 3M-line commercial target) it is
...
retention and hands the analysis real headroom — exactly the closed-core lever.

Residual risk for closed-core: its dependency surface differs, so a few residual misses are possible;
...
### closed-core outcome (the target, 2026-07-22)

closed-core is 23,394 types / 661,633 elements (2.65× full ES) and is one giant SCC. With `DROP_AST=1`
+ single-pass + flatten:
...
- **Live histogram at the 48% wall: 29.4G** — dominated by prepwork VariableData ~7.2G (closed-core's
...
**Settled: ~44G is the closed-core requirement with today's levers (drop-AST + single-pass + flatten);
