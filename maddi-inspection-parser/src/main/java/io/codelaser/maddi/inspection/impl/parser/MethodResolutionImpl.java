...
import static io.codelaser.maddi.inspection.api.parser.TypeContext.SUBTYPE_HIERARCHY_IN_CONSTRUCTOR_PRIORITY;
import static io.codelaser.maddi.inspection.impl.parser.ListMethodAndConstructorCandidates.IGNORE_PARAMETER_NUMBERS;
import static io.codelaser.maddi.inspection.impl.parser.ReevaluateErasedExpressions.containsErasedExpressions;
...
// we need the intermediate step to original because the result of translateMap contains E=#0 in Collection
    ...
    private int computeScoreForReevaluationOfErasedParameterExpressions(Context context,
        ...
        for (int i : erased) {
            ...
            ReevaluateErasedExpressions.ReEval res = reEval.reevaluateParameterExpression(context, index,
                    unparsedArguments, outsideContext, method.methodInfo().name(), evEx, method, i, cumulative,
                    parameters);
            cumulative = res.typeParameterMap();
            ParameterizedType pt;
