...
.. finally, the `AnalysisHintsComposer` and `DecoratorImpl` are used to write out the files.
...
.. again, the `AnalysisHintsComposer` and `DecoratorImpl` are used to overwrite the files.
