...
public class MarkerVariable extends LocalVariableImpl implements LocalVariable, LinkVariable {
    ...
    MarkerVariable(String suffix, ParameterizedType parameterizedType, Expression assignmentExpression) {
        super(PREFIX + suffix, parameterizedType, assignmentExpression);
    }

    public static MarkerVariable constant(int index, ParameterizedType parameterizedType, Expression expression) {
        return new MarkerVariable(LABEL_CONSTANT + index, parameterizedType, expression);
    }

    public static MarkerVariable someValue(Runtime runtime, ParameterizedType parameterizedType) {
        return new MarkerVariable(LABEL_SOME_VALUE, parameterizedType, runtime.newEmptyExpression());
    }
