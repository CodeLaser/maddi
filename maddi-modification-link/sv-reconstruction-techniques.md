...
- **Trust only full-suite A/B** for attribution; env gates exist for every major mechanism
  ...
  NOPDEEP NOSIBR NOVMIDIR NOFLIPSAME NORVM NOSIBFACE NOSIBEQ NOFLIPOWN NORVEQ NORVSP NOVARTO`). Debug aids: `SVDUMP` (per-statement group dump), `SBDUMP=<simpleName>`
