# sv engine journal (frozen, append-only)

Reverse-chronological engineering journal of the sv-integration work: corpus certification,
worklist/parallelism, profiling, audits, first contacts. Split out of `sv-remaining-catalogue.md`
on 2026-07-18 when that file's catalogue function had long since closed (see "catalogue CLOSED"
below). The LIVE state and open items are in `sv-remaining-catalogue.md`; new journal entries
go THERE first and are periodically moved here.
...
## UPDATE 2026-07-18f — ELASTICSEARCH FIRST CONTACT (in progress)

Scanner bugs found+FIXED by rounds 4-6 (each suite-gated; repros in
maddi-java-openjdk TestElasticsearchShapes unless noted):
1. byte[]::new (primitive-array constructor reference): qualifier scan yields no expression
   -> NPE on evaluatedScope. Now derives the array type from javac's own type.
2. Type$ForAll -> MethodType cast in visitMemberReference (generic SAM descriptor,
   TransportService): unwrap forAll.qtype, same idiom as ClassSymbolScanner.methodType.
3. 'Duplicating type $13.BytesCompareUnsigned': member record of an ANONYMOUS class,
   forward-referenced -> resolution through a stub owner missed the source-registered type;
   lazilyLoadTypeFromClassFile now reuses the InfoByFqn entry under the derived FQN.
4. 'Extending multiple identical interfaces' on the same shape one layer up: the lazy path
   re-ran the class-symbol setup on a SOURCE-scanned anonymous type; the source scan now
   marks classScannerSetupDone at anonymous registration.
5. commitParameters 'Already set' (ReaperService.Params): a still-open method builder
   re-committed when a type is both source-scanned and lazily COMPLETE-loaded from bytecode
   (build-tools is a compiled dependency of build-tools-internal). Made idempotent for an
   unchanged FQN; a DIFFERENT FQN still trips.
OPEN (same dual-identity family): bare UOE scanning setInternal of a 'Create multi'
re-registered type — source-set re-scan of an already-loaded type touches a committed
builder. Scoped around for now by dropping the build-tooling source sets (build-conventions,
reaper, build-tools, build-tools-internal) — they were never the stress target; the corpus
is server + libs + client + test framework (27 sets). Investigate properly with a two-source-
set unit repro (A compiles to classes dir, B depends on it, both parsed from source).

Corpus: server-module closure of elastic/elasticsearch (gradle project, 5.1M lines total;
server+libs ~34 source sets). Config derived via the compile-log pipeline: `./gradlew
:server:compileTestJava --rerun-tasks --debug | grep "Compiler arguments"` (40 invocations)
→ `--compile-log ... --write-input-configuration ...`. Two config-level findings:
- --extra-jmod=jdk.incubator.vector needed (simdvec / Panama vector API).
- BUG (engine, CompileListToSourceSets): libs/native has TWO -d destinations (classes/java/
  main + generated-foreign-library-classes); the naming picked 'build/main' from the second
  while dependency EDGES point at 'native/main' → consumers silently lose the dep, 46x
  "package org.elasticsearch.nativeaccess does not exist". Post-processed (rename) for the
  stress run; the durable fix is naming from the classes/java/<name> destination only.
- MRJAR overlay variants (entitlement main25/26/27, cli-terminal/foreign-adapter main22)
  become duplicate-FQN source sets ('java/mainN'); dropped in post-processing. Open design
  question: multi-release source sets in the config model.
