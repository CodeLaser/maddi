...
**Start here (routing):**
- Working on LINKING (call sites, summaries, FI)? → `linking-manual.md`, start at §5 + §6; spec-by-example
  is `src/test/.../impl/TestLinkMethodCall.java`.
- Working on the SV ENGINE (collapse, reconstruction, worklist, certification)? → `sv-engine-handoff.md`
  (entry point), then `sv-reconstruction-techniques.md`.
- Notation / operator table? → this file + `operator.adoc` (generated by `TestLinkNature`).
- Virtual fields (`§`)? → `src/main/java/.../vf/virtual-fields.md`.
- Terminology (mutable/immutable/independent/hidden content)? → `road-to-immutability/llm-summary.md`.

A link is a triple `(from, linkNature, to)`, where `from` and `to` are variables, and `linkNature` is one of:

* ≡ _is identical to_ (a `☷`-suffixed pass-variant of ≡ carries the modifying methods it survived; print-only in summaries)
* ← _is assigned from_ / → _is assigned to_
...
* ∈? / ∋? _could be element of / could contain_: tentative element relations (e.g. unresolved indexing)
* ↗ _is decorated with_ / ↖ _contains decoration_: functional-interface application markers
...
1. intermediate variables (`$__` prefix), for variables that cannot escape local linking
    1. `$__rv` return value of a method call (currently ALSO used for parameter stand-ins — misleading in traces)
    2. `$__c` newly created object
    3. `$__ic` inline condition
    4. `$__sv_` shared variable: the representative of an assignment group in the collapsed graph
       (class `SharedVariable`, see `sv-reconstruction-techniques.md`)
2. marker variables: (`$_` prefix)
    1. `$_fi` functional interface variable (class `FunctionalInterfaceVariable`): a captured lambda /
       method reference with its links
    2. `$_afi` applied functional interface (class `AppliedFunctionalInterfaceVariable`): a SAM applied to a
       not-yet-known lambda, resolved by the caller that supplies it
    3. `$_ce` constant expression; useful for array indexing
    4. `$_v` some value; used to help identify `@Identity`

`Λ` is NOT a variable kind: it is a print-time decoration on any FI-typed link target (prepwork `LinksImpl`).
