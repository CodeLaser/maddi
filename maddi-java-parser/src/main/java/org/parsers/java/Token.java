...
public class Token implements CharSequence, Node.TerminalNode { 
    ...
    public enum TokenType implements Node.NodeType { 
        EOF, CTRL_Z(""), EXPORTS("exports", true, false), MODULE("module", true, false),
        OPEN("open", true, false), OPENS("opens", true, false), PERMITS("permits", true, false),
        PROVIDES("provides", true, false), RECORD("record", true, false), REQUIRES("requires", true, false),
        SEALED("sealed", true, false), TO("to", true, false), TRANSITIVE("transitive", true, false),
        USES("uses", true, false), VAR("var", true, false), WHEN("when", true, false),
        WITH("with", true, false), YIELD("yield", true, false), UNDERSCORE("_"), ABSTRACT("abstract"),
        _ASSERT("assert"), BOOLEAN("boolean"), BREAK("break"), BYTE("byte"), CASE("case"),
        CATCH("catch"), CHAR("char"), CLASS("class"), CONST("const"), CONTINUE("continue"),
        _DEFAULT("default"), DO("do"), DOUBLE("double"), ELSE("else"), ENUM("enum"),
        EXTENDS("extends"), FALSE("false"), FINAL("final"), FINALLY("finally"), FLOAT("float"),
        FOR("for"), GOTO("goto"), IF("if"), IMPLEMENTS("implements"), _IMPORT("import"),
        ...
        SYNCHRONIZED("synchronized"), _THIS("this"), THROW("throw"), THROWS("throws"),
        TRANSIENT("transient"), TRUE("true"), TRY("try"), VOID("void"), VOLATILE("volatile"),
        WHILE("while"), NON_SEALED("non-sealed"), LPAREN("("), RPAREN(")"), LBRACE("{"),
        RBRACE("}"), LBRACKET("["), RBRACKET("]"), SEMICOLON(";"), COMMA(","), DOT("."),
        DOUBLE_COLON("::"), VAR_ARGS("..."), AT("@"), ASSIGN("="), GT(">"), LT("<"),
        BANG("!"), TILDE("~"), HOOK("?"), COLON(":"), EQ("=="), LE("<="), GE(">="),
        NE("!="), SC_OR("||"), SC_AND("&&"), INCR("++"), DECR("--"), PLUS("+"), MINUS("-"),
        STAR("*"), SLASH("/"), BIT_AND("&"), BIT_OR("|"), XOR("^"), REM("%"), LSHIFT("<<"),
        PLUSASSIGN("+="), MINUSASSIGN("-="), STARASSIGN("*="), SLASHASSIGN("/="),
        ANDASSIGN("&="), ORASSIGN("|="), XORASSIGN("^="), REMASSIGN("%="), LSHIFTASSIGN("<<="),
        RSIGNEDSHIFT(">>"), RUNSIGNEDSHIFT(">>>"), RSIGNEDSHIFTASSIGN(">>="), RUNSIGNEDSHIFTASSIGN(">>>="),
    ...
    */
    public static Token newToken(TokenType type, TokenSource tokenSource, int beginOffset, int endOffset) { 
        switch(type) { 
            ...
            case _THIS : 
                return new ThisLiteral(TokenType._THIS, tokenSource, beginOffset, endOffset);
            case IF : 
