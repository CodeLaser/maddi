...
| [handoff-isolateclass-enum-and-generic-stubs.md](handoff-isolateclass-enum-and-generic-stubs.md) | plan | Two `IsolateClass` stub defects, each a single decision in `IsolationCore`, each with a failing driver in `TestIsolateClass4Compiles`: an **enum stubbed as a class** (line ~184 reproduces the annotation and interface natures and lets enum fall through, so `case CLASS:` gets "pattern or enum constant required" — the fix needs `typeNatureEnum` plus constants as *synthetic* fields, which is how `TypePrinterImpl.enumConstantStream` recognises them), and a **method inherited from a generic supertype stubbed on the erased scope type** (lines 903–906, so `ListStack<T>.pop()` becomes `Object pop()` on the subtype — the field branch thirty lines below already places inherited members on their declaring type and says why). Found by compiling fernflower isolates through the new `debug.isolateClass`; invisible before because the isolate tests re-parse with `failFast` false, where javac's errors are logged and carried. §4 records the third defect of the set, **fixed** (`9bacea3b`): an interface field was static only if it had reached maddi from the classpath, and why the JLS rule had to go at the parse site rather than into `FieldInfo.isStatic()` — four `TestGetSet` cases lost variable identity when it went into the accessor, maddi modelling synthetic *instance* fields on interface types. |
