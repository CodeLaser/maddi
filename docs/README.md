...
| [handoff-overrides-same-compilation-unit.md](handoff-overrides-same-compilation-unit.md) | note | `overrides()` comes back **empty for every method** when several top-level types share one compilation unit -- legal Java, and not limited to the multiple-interface case: a method implementing a single interface declared three lines above it reports nothing either. Breaks any consumer that groups members by implemented interface (jfocus splitclass does), silently rather than with an error. Reproducer is in the repo, `@Disabled`, next to two passing tests that differ only in using three compilation units instead of one. Records the trap: this shape imitates "overrides() lost an interface" exactly, and is not that. **Still open.** **Amended 2026-07-28:** a *second* override defect was found and **fixed** the same day — an uncommitted method's FQN was `?.?.<name>`, and since the FQN is a method's identity, two same-signature interface methods compared equal while scanning and the `Set` of overrides kept one. That one caused the splitclass non-determinism, and it means this file's original claim that "the two-interface behaviour is correct and deterministic" was wrong; the corrected trap section says why the single-order tests missed it. See `TestOverridesUnitOrder`. |
