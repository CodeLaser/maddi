...
| [handoff-from-jfocus-standardize.md](handoff-from-jfocus-standardize.md) | note | Three defects found from downstream: `PART_OF_CONSTRUCTION` marks call-graph-reached types as prepped (trips the link computer's `assert vd != null`), `ClassSymbolScanner.convert` has no `BottomType` case, and `ImportComputer` never emits static imports so printed source does not re-parse. Plus a request to disambiguate the two meanings of `NYI`. |
