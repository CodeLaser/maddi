...
| [handoff-isolatemethod-remaining.md](handoff-isolatemethod-remaining.md) | note | The three `IsolateMethod` issues left after the closed-core hardening round (37 of 39 isolates now parse back, up from 26): a simple-name collision between two JDK types where the wrong one wins the import (`java.util.Date` vs `java.sql.Date`), a stub in a namespace chain referenced both fully-qualified and simply (a *printing* problem, not a placement one), and a `MethodPrinterImpl` NPE on a bare `return;`. Each diagnosed to root cause, with what was tried and reverted, and the baseline numbers. |
