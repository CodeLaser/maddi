...
| [method-type-parameter-source-loss.md](method-type-parameter-source-loss.md) | note | **Investigation, nothing fixed.** A method's type parameters are built twice — once by `ClassSymbolScanner` from the javac symbol when a caller is reached first, once by `ScanCompilationUnit` from the declaration — and the symbol-built ones win, because `MethodInfo.Builder` offers only `addTypeParameter` where `TypeInfo.Builder` offers `addOrSetTypeParameter`. Committed without a source and with bounds widened by `withWildcard(wildcardExtends())`, so the same file yields a **different CST depending on scan order**: `tp.source()` null (nothing can locate the `<T ...>` token) and the bound `? extends Score<Score_>` instead of `Score<Score_>`. Measured at **38% of method type parameters on langchain4j, 43% on timefold** — the ordinary case, not an edge case. The FIXME is already at the spot, and the fix is symmetric with `deferParameterCommit`, which solves the identical hazard for parameters one loop below. Confirmed to break rename-type-parameter (silently: uses renamed, declaration left); the open question is whether the differing bound reaches any analysis verdict. |
