...
| [method-type-parameter-source-loss.md](method-type-parameter-source-loss.md) | note | **Resolved 2026-08-01.** A method's type parameters are built twice — once by `ClassSymbolScanner` from the javac symbol when a caller is reached first, once by `ScanCompilationUnit` from the declaration — and the symbol-built ones win, because `MethodInfo.Builder` offers only `addTypeParameter` where `TypeInfo.Builder` offers `addOrSetTypeParameter`. Committed without a source and with bounds widened by `withWildcard(wildcardExtends())`, so the same file yields a **different CST depending on scan order**: `tp.source()` null (nothing can locate the `<T ...>` token) and the bound `? extends Score<Score_>` instead of `Score<Score_>`. Measured at **38% of method type parameters on langchain4j, 43% on timefold** — the ordinary case, not an edge case. The FIXME is already at the spot, and the fix was symmetric with `deferParameterCommit`, which solved the identical hazard for parameters one loop below: defer everything to the declaration, which then supplies annotations, bounds and source together. It had broken rename-type-parameter silently (uses renamed, declaration left). §7 records what landed and why the guard is `hasBeenInspected()` rather than a null source; the 8% of CLASS type parameters that also lack a source is a different route and stays open. |
