...
| [semantic-preconditions-for-relocation.md](roadmap/semantic-preconditions-for-relocation.md) | plan (**parked** 2026-09-25) | Design for §3.1 of the applications roadmap, backed by real cases: move static members committed three OpenSearch changes that break at class initialisation (a settings holder that cannot load; defaults read as `null`/`0`), and method object turns Pulsar's `RateLimiter.acquire` into a deadlock — all reproduced. The two evidenced checks (static read-before-init, monitor methods bound to the wrong object) need the CST and call graph, not link analysis; link-based checks cover the unevidenced tail. Every check returns *found / clean / not checked*. |
